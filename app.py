...
def index():
    ...
    geolocated_pollution_data = clean_and_reformat(pollution_data, station_data)

    # the data file is large, so take a sample
    maxReading, sample_pln_data = sample_data(geolocated_pollution_data)

    # data needs to be sorted into time series for use with HeatMapWithTime
    # sort_data_into_time_series()
...
def clean_and_reformat(pollution_data, station_data):
    ...
    geolocated_pollution_data = geolocated_pollution_data.rename(
        columns={"Site Name": "sitename", "Reading": "reading", "Latitude": "latitude", "Longitude": "longitude"},
        errors="raise")
    ...
    return geolocated_pollution_data

def sort_data_into_time_series():
...
def sample_data(geolocated_pollution_data):
    ...
    return maxReading, sample_pln_data
